    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="mt-8 bg-white overflow-hidden shadow sm:rounded-lg p-6">
        <h2 class="text-2xl leading-7 font-semibold">
          HMR + JIT bug reproducer repo
        </h2>
        <p class="mt-3 text-gray-600">
          When JIT engine encounters new (i.e. "ungenerated") class, it has to generate it during rebuild, and then
          push regenerated CSS to the browser. Unfortunately when doing so it resets the state of the whole page.
          Moreover, it triggers full browser page refresh (instead of just re-instantiating the component).
          Whole rich HMR experience is efficiently degraded to "automatic F5 hitter".
        </p>
        <p class="mt-3 text-gray-600">
          State reset indications: counter = {{ counter }}
          <button @click="counter += 1" class="border border-gray-200 bg-gray-100 rounded px-1 py-0.5 text-xs">+1</button>
          ; physical page loading time = {{ loadingTime }}
        </p>
        <p class="mt-4 pt-4 text-gray-800 border-t border-dashed">
          <span class="text-indigo-500">This is a test text. Change class to any other color you like to trigger the bug.</span>
        </p>
        <p class="mt-4 pt-4 text-gray-800 border-t border-dashed">
          <span class="text-blue-500">This is another test text. When you change color of the above text to blue,
            no state reset occurs since blue class is already generated.</span>
        </p>
      </div>
    </div>
  </div>
...
export default {
  ...
  beforeMount() {
    // Indication of full page reload, opposed to just re-instantiation of component.
    this.loadingTime = performance.timing.navigationStart;
  }
}
</script>
